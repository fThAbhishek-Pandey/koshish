      ...
      return (
          <div className="min-h-screen bg-green-50 flex items-center justify-center relative top-24 mb-32">
              <div className="max-w-4xl w-full bg-green-100 shadow-xl rounded-lg p-10 flex flex-col items-center">
                  ...
                  {/* Name & Speciality */}
                  <h2 className="text-4xl font-bold text-blue10 mt-6">{myMentor.name}  <span className='text-md text-blue-400 font-light'>{myMentor.yog-4}-{myMentor.yog-2000}</span></h2>
                  <p className="text-blue-700 text-lg font-medium mt-2">{myMentor.speciality}</p>
                  
                  {/* Quote */}
                  {myMentor.quote != "NAN" && <blockquote className="relative text-gray-600 text-lg italic leading-relaxed bg-blue-100 p-6 mt-6 rounded-lg shadow-md text-center w-full max-w-2xl">
                ...
                <div className="mt-8 w-full text-center px-6">
                    { myMentor.aboutHead !="NAN" &&  <h3 className="text-3xl font-bold text-blue10 underline decoration-blue-500 decoration-4 underline-offset-4">{myMentor.aboutHead}</h3>}
                    { myMentor.about !="NAN" && <p className="text-gray-700 text-lg mt-4 leading-relaxed bg-gray-100 p-4 rounded-lg shadow-sm">{myMentor.about}</p>}
                </div>
                
                  
                  {/* Details Section */}
                  <div className="mt-8 w-full text-center">
                     { myMentor.subject !="NAN" && <p className="text-gray-800 text-lg font-semibold">📖 Subject: {myMentor.subject}</p>}
                     {myMentor.classTeacher !="NAN" && <p className="text-gray-800 text-lg">👨‍🏫 Class Teacher: {myMentor.classTeacher} </p>}

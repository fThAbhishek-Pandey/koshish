  ...
  if (!idEvent) {
    return (
      <div className="flex items-center justify-center min-h-[40vh]">
        <p className="text-gray-500 text-lg animate-pulse">Loading event details...</p>
      </div>
    );
  }

  return (
    <div className="flex flex-col items-center px-4 py-10 bg-gradient-to-b from-green-50 to-green-100 min-h-screen">
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 text-sm sm:text-base text-gray-600 mb-4">
            <p><span className="font-medium text-gray-700">Starts:</span> {new Date(idEvent.startdate).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</p>
            <p><span className="font-medium text-gray-700">Ends:</span> {new Date(idEvent.endDate).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</p>
            <p><span className="font-medium text-gray-700">Mode:</span> {idEvent.mode ? "Online" : "Offline"}</p>
          </div>
          ...
          {/* Prizes (if available) */}
          {/* Description */}
          {idEvent.prize && <div className="prose max-w-none text-gray-800 mb-6">
         ...
         { idEvent.prize && <div className="prose max-w-none text-gray-800 mb-6">
          ...
          {idEvent.prize && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
              <h3 className="text-lg font-semibold text-yellow-800 mb-2">{idEvent.prize.PrizeHeading}</h3>
          ...
          {idEvent.participants && (
            <p className="text-center text-gray-600 text-sm mb-4">
              👤 <span className="font-medium">{idEvent.participants}</span> Registered
            </p>
          )}
